...
export default function InputBox({ input, isLoading, onChange, onSend, centered }: Props) {
  ...
  return (
    <div className={`shrink-0 pb-3 pt-2 ${centered ? '' : 'border-t border-white/[0.05]'}`}>

      {/* Input row — max width centered like Claude */}
      <div className="mx-auto w-full max-w-2xl px-4">
        <div className="flex gap-0 items-center bg-white/[0.05] border border-white/[0.10] rounded-2xl overflow-hidden focus-within:border-emerald-500/50 transition-colors shadow-lg">
          <textarea
            ...
            onInput={handleInput}
            disabled={isLoading}
            placeholder={isLoading ? 'Agent is responding…' : 'Ask anything about your order…'}
            rows={1}
            maxLength={2000}
